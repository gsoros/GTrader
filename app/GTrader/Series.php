...
class Series extends Collection
{
    ...
    public function sma($len = 0, $price = 'close')
    {
        $len = intval($len);
        if ($len <= 1) {
            throw new \Exception('sma needs int len > 1');
        }
    ...
    /** Bollinger Bands */
    public function bb($len = 20, $stdev = 2, $price = 'close')
    {
        $len = intval($len);
        if ($len <= 1) {
            throw new \Exception('bb needs int len > 1');
        }
        $stdev = floatval($stdev);
        if ($stdev < 1) {
            throw new \Exception('bb needs float stdev >= 1');
        }
        if (!in_array($price, array('open', 'high', 'low', 'close'))) {
            throw new \Exception('bb needs valid price');
        }

        $indicator1 = 'bb_high_'.$len.'_'.$stdev.'_'.$price;
        $indicator2 = 'bb_low_'.$len.'_'.$stdev.'_'.$price;
        if (isset($this->indicators[$indicator1]) && isset($this->indicators[$indicator2])) {
            return $this;
        }
        $this->indicators[$indicator1] = $this->indicators[$indicator2] = true;
        ...
        while ($candle = $series->next()) {
            ...
            if (count($dev_sq_arr) > $len) {
                array_shift($dev_sq_arr);
            }
